...
2. **Register the key on GitHub.** When a key is generated, copy the public half and click the link to add it at *GitHub → Settings → SSH keys*.
...
4. **Switch.** Pick a profile and hit **Activate** — this updates your global `user.name` / `user.email` and the active SSH identity. The active profile is shown on the dashboard.
