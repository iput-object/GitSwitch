...
export default function SelectProviderStage({
  ...
  return (
    ...
    >
      ...
      <motion.p
        variants={item}
        className="relative max-w-85 text-sm leading-relaxed text-neutral-400 mb-8"
      >
      ...
      <motion.div
        variants={item}
        className="w-full max-w-lg grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6"
      >
        {providers.map((p) => (
          <button
            key={p.id}
            onClick={() => onSelect(p.id)}
            className="group relative flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/3 text-left transition-all hover:bg-white/6 hover:border-white/10 overflow-hidden"
          >
        ...
        <button
          ...
          className="group relative flex items-center justify-center gap-2 p-4 rounded-xl border border-dashed border-white/10 bg-transparent text-neutral-400 transition-all hover:bg-white/2 hover:border-white/20 hover:text-neutral-200"
        >
